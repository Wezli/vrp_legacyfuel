		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Saia do veículo para abastecer o veículo.")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Pressione ~g~G~w~ para cancelar o abastecimento do seu veículo. ~r~R$" .. price .. "~w~.")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Seu veículo já está abastecido.")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Você não tem dinheiro suficiente para abastecer.")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Pressione ~g~G~w~ para abastecer o veículo. ~r~R$2.00~w~/L.")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Pressione ~g~G~w~ para cancelar o abastecimento do veículo. Gasolina: " .. fuel .. "%")
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Pressione ~g~G ~w~para abastecer o veículo com seu galão.")
...
Citizen.CreateThread(function()
	while true do
		...
		if IsFueling then
			...
			if cash >= price then
				TriggerServerEvent('LegacyFuel:CheckServerFuelTable', plate, vehicleModel)
				Citizen.Wait(150)
				...
				if newfuel < 100 then
					...
					for i = 1, #Vehicles do
						if Vehicles[i].plate == plate and Vehicles[i].vmodel == vehicleModel then
							TriggerServerEvent('LegacyFuel:UpdateServerFuelTable', vehicleModel, plate, round(GetVehicleFuelLevel(vehicle), 1))
	...
	while true do
		...
		if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
			...
			if not found then
				...
				TriggerServerEvent('LegacyFuel:UpdateServerFuelTable', vehicleModel, plate, fuel)
			end
...
function DisplayHud()
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		...
		if rpm > 0.9 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 255, 0, 0, 255, 4, 1)
		elseif rpm > 0.8 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 255, 75, 0, 255, 4, 1)
		elseif rpm > 0.7 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 255, 150, 0, 255, 4, 1)
		elseif rpm > 0.6 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 255, 200, 0, 255, 4, 1)
		elseif rpm > 0.5 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 255, 255, 0, 255, 4, 1)
		elseif rpm > 0.4 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 200, 255, 0, 255, 4, 1)
		elseif rpm > 0.3 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 150, 255, 0, 255, 4, 1)
		elseif rpm > 0.2 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 100, 255, 0, 255, 4, 1)
		elseif rpm <= 0.1 then
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 0, 255, 0, 255, 4, 1)
		else
			DrawAdvancedText(0.245 - UI.x, 0.77 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 0, 255, 0, 255, 4, 1)
		end
	end
			...
			for i = 1, #Vehicles do
				if Vehicles[i].plate == plate and Vehicles[i].vmodel == vehicleModel then
					...
					if updatedfuel ~= 0 then
						TriggerServerEvent('LegacyFuel:UpdateServerFuelTable', vehicleModel, plate, updatedfuel)
...
RegisterNetEvent("LegacyFuel:buyCan")
AddEventHandler("LegacyFuel:buyCan", function()
	GiveWeaponToPed(GetPlayerPed(-1), 0x34A67B97, 4500, false, true)
end)
